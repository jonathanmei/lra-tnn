        ...
        for batch in super().__iter__():
            ...
            z = [
                torch.cat(
                    [
                        torch.zeros(
                            (z[i].shape[0], self.overlap_len - 1, *z[i].shape[2:])
                        )
                        .to(z[i].device)
                        .to(z[i].dtype),
                        z[i],
...
class CIFAR10(SequenceDataset):
    ...
    def setup(self):
        ...
        if self.augment:
            ...
            augmentations = []
        torchvision.transforms_train = augmentations + preprocessors + permutations
...
class AAN(SequenceDataset):
    ...
    def process_dataset(self):
        ...
        dataset = dataset.map(
            ...
            num_proc=max(self.n_workers, 1),
            cache_file_names={
                "train": "/home/ubuntu/jmei/lra_release/aan/tmp/train.arrow",
                "val": "/home/ubuntu/jmei/lra_release/aan/tmp/val.arrow",
                "test": "/home/ubuntu/jmei/lra_release/aan/tmp/test.arrow",
            },
        )
        vocab = torchtext.vocab.build_vocab_from_iterator(
